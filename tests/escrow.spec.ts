...
const NETWORK_FEE = toNano("0.055"); // reimburses deploy + action gas (reduced budget)
...
async function testTonDeal() {
  ...
  assert(sellerDelta > amount, `seller must receive the full amount plus unused gas refund, got delta ${sellerDelta} vs amount ${amount}`);
  assert(sellerDelta <= amount + maxPossibleExcess, `seller's refund cannot exceed the total prepaid gas pool, got excess ${sellerDelta - amount}`);
