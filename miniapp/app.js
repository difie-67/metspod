(() => {
  const UI_BUILD_ID = "2026-09-24_00-20_dealux7";
  ...
  function calculatorVariants(units) {
    ...
    return `<div class="calc-brief">
      ...
      <div class="calc-result-row"><div><span>Комиссия удержится из выплаты продавцу</span><strong>${esc(fromUnits(sellerRows.total))} TON покупатель · ${esc(fromUnits(sellerRows.sellerPayout))} TON продавец</strong></div><em>−${esc(fromUnits(sellerRows.serviceFee))} TON</em></div>
    </div>`;
  }
  ...
  function heroSiteMapMarkup() {
    return `<nav class="hero-site-map" aria-label="Быстрые разделы">
      <button data-home-anchor="trust">О безопасности</button><button data-home-anchor="how-it-works">Как работает</button><button data-home-anchor="journey">Этапы сделки</button><button data-home-anchor="active-deals">Активные</button><button data-home-anchor="public-deals">Реестр</button><button data-home-anchor="pricing">Калькулятор</button>
    </nav>`;
  }
  ...
  function homeView() {
    ...
    return `<section class="hero-open"><img class="hero-mark" src="/miniapp/logo-mark.png" alt="" aria-hidden="true" width="56" height="56" /><p class="hero-eyebrow">TON · SMART CONTRACT ESCROW</p><h1>Безопасные<br>сделки в TON</h1><p>Деньги хранятся в отдельном смарт-контракте сделки и выпускаются только по её правилам.</p><div class="hero-actions"><button class="btn btn-primary" data-route="create">Создать сделку</button><button class="btn btn-secondary" data-anchor="trust">Боюсь, что скам</button></div>${heroSiteMapMarkup()}</section>
      <div class="stats home-stats">
        <article class="stat stat-primary" data-no="01"><div class="stat-top"><span class="stat-index">01</span><span class="stat-signal"><b></b><b></b><b></b></span></div><strong data-count="${active.length}">${active.length}</strong><span class="stat-label">Активные<br>сделки</span><i class="stat-orbit"></i></article>
        <article class="stat stat-solid" data-no="02"><div class="stat-top"><span class="stat-index">02</span><span class="stat-signal"><b></b><b></b><b></b></span></div><strong data-count="${completed.length}">${completed.length}</strong><span class="stat-label">Завершено<br>успешно</span><i class="stat-grid"></i></article>
        <article class="stat stat-light" data-no="03"><div class="stat-top"><span class="stat-index">03</span><span class="stat-signal"><b></b><b></b><b></b></span></div><strong>0.6–2%</strong><span class="stat-label">Комиссия платформы<br>по сетке</span><i class="stat-cross">＋</i></article>
      </div>
      ${trustSection()}
      ${sectionDivider()}
      ${howItWorksSection()}
  ...
  function adminDiagRows(diag) {
    if (!diag) return `<div class="empty">Диагностика пока недоступна</div>`;
    const sourceHashes = diag.contract?.sourceHashes || {};
    const logRows = (diag.logs || []).map((entry) => `<div class="admin-log-line"><span>${esc(entry.ts.replace("T", " ").replace(".000Z", "Z"))}</span><b>${esc(entry.level.toUpperCase())}</b><code>${esc(entry.message)}</code></div>`).join("") || `<div class="empty">Логов пока нет</div>`;
    return `
      <section class="plain admin-diagnostics">
        <div class="section-head section-head-tight"><h2>Сервис</h2><button class="text-button" data-admin-refresh>Обновить</button></div>
        <dl class="kv kv-plain admin-kv">
          <dt>Build</dt><dd><button class="copy-value" data-copy="${esc(diag.buildId)}" data-copy-label="Build ID скопирован">${esc(diag.buildId)}</button></dd>
          <dt>Запущен</dt><dd>${esc(diag.startedAt || "—")}</dd>
          <dt>Сеть</dt><dd>${esc(diag.network || "—")}</dd>
          <dt>Mini App</dt><dd>${diag.miniAppUrl ? `<a href="${esc(diag.miniAppUrl)}" target="_blank" rel="noopener">${esc(diag.miniAppUrl)}</a>` : "—"}</dd>
          <dt>RPC</dt><dd>${esc(diag.tonEndpoint || "—")}</dd>
          <dt>Service wallet</dt><dd><button class="copy-value" data-copy="${esc(diag.serviceWallet?.address || "")}" data-copy-label="Адрес скопирован">${esc(diag.serviceWallet?.address || "—")}</button></dd>
          <dt>Баланс</dt><dd>${esc(diag.serviceWallet?.balanceTon || "0")} TON · ${esc(diag.serviceWallet?.state || "—")}</dd>
          <dt>Fee wallet</dt><dd><button class="copy-value" data-copy="${esc(diag.platformAddress || "")}" data-copy-label="Адрес комиссии скопирован">${esc(diag.platformAddress || "—")}</button></dd>
        </dl>
        <div class="link-list admin-links"><a href="${esc(diag.serviceWallet?.explorers?.tonviewer || "#")}" target="_blank" rel="noopener">Service wallet в Tonviewer ↗</a><a href="${esc(diag.serviceWallet?.explorers?.tonscan || "#")}" target="_blank" rel="noopener">Service wallet в Tonscan ↗</a>${diag.verifierUrl ? `<a href="${esc(diag.verifierUrl)}" target="_blank" rel="noopener">Верификация контракта ↗</a>` : ""}</div>
      </section>
      <section class="plain admin-diagnostics">
        <div class="section-head section-head-tight"><h2>Контракт и хэши</h2></div>
        <dl class="kv kv-plain admin-kv">
          <dt>Code hash</dt><dd><button class="copy-value" data-copy="${esc(diag.contract?.codeHash || "")}" data-copy-label="Code hash скопирован">${esc(diag.contract?.codeHashShort || diag.contract?.codeHash || "—")}</button></dd>
          <dt>contracts/escrow.tact</dt><dd><button class="copy-value" data-copy="${esc(sourceHashes.escrowTact || "")}" data-copy-label="Hash скопирован">${esc(diag.contract?.sourceHashesShort?.escrowTact || "—")}</button></dd>
          <dt>contracts/wrappers/Escrow.ts</dt><dd><button class="copy-value" data-copy="${esc(sourceHashes.wrapperTs || "")}" data-copy-label="Hash скопирован">${esc(diag.contract?.sourceHashesShort?.wrapperTs || "—")}</button></dd>
          <dt>tact.config.json</dt><dd><button class="copy-value" data-copy="${esc(sourceHashes.tactConfig || "")}" data-copy-label="Hash скопирован">${esc(diag.contract?.sourceHashesShort?.tactConfig || "—")}</button></dd>
          <dt>package.json</dt><dd><button class="copy-value" data-copy="${esc(sourceHashes.packageJson || "")}" data-copy-label="Hash скопирован">${esc(diag.contract?.sourceHashesShort?.packageJson || "—")}</button></dd>
        </dl>
        ${diag.buildNotes ? `<details class="admin-build-notes"><summary>Что сверять при деплое</summary><pre>${esc(diag.buildNotes)}</pre></details>` : ""}
      </section>
      <section class="plain admin-diagnostics">
        <div class="section-head section-head-tight"><h2>Логи рантайма</h2></div>
        <div class="admin-logs">${logRows}</div>
      </section>`;
  }

  function adminView() {
    if (!state.admin) return `<div class="error-box">Нет доступа</div>`;
    const stats = state.admin.stats;
    const diagnostics = state.admin.diagnostics;
    return `<h1 class="page-title">Админ-панель</h1><p class="page-intro">Контроль сделок, служебного кошелька и быстрых технических данных прямо из Mini App.</p><div class="stats home-stats admin-stats"><article class="stat stat-primary" data-no="01"><div class="stat-top"><span class="stat-index">01</span><span class="stat-signal"><b></b><b></b><b></b></span></div><strong>${stats.users}</strong><span class="stat-label">Пользова-<br>телей</span><i class="stat-orbit"></i></article><article class="stat stat-solid" data-no="02"><div class="stat-top"><span class="stat-index">02</span><span class="stat-signal"><b></b><b></b><b></b></span></div><strong>${stats.active}</strong><span class="stat-label">Активных<br>сделок</span><i class="stat-grid"></i></article><article class="stat stat-light" data-no="03"><div class="stat-top"><span class="stat-index">03</span><span class="stat-signal"><b></b><b></b><b></b></span></div><strong>${stats.disputed}</strong><span class="stat-label">Открытых<br>споров</span><i class="stat-cross">＋</i></article></div>${adminDiagRows(diagnostics)}<div class="section-head"><h2>Последние сделки</h2></div><div class="card-list">${state.admin.recent.map(dealCard).join("") || `<div class="empty">Сделок нет</div>`}</div>`;
  }
  ...
  function dealView(deal) {
    ...
    return `<button class="text-button deal-back" data-route="deals">← Все сделки</button>
      <section class="detail-head detail-head-open">
        ...
        <h1>${esc(deal.amountTon)} <small>TON</small></h1><span class="detail-ghost-amount" aria-hidden="true">${esc(deal.amountTon)}</span>
  ...
  function bindCurrentView() {
    ...
    view.querySelectorAll("[data-admin-refresh]").forEach((button) => button.onclick = () => refresh());
