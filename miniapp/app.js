(() => {
  const UI_BUILD_ID = "2026-09-24_15-18_dealux8_stylepass";
  console.info("OBRA UI BUILD", UI_BUILD_ID);
  "use strict";
  ...
  let state = { user: null, config: null, deals: [], admin: null, adminDiagnostics: null, route: "home", selectedDeal: null };
  ...
  function calculatorVariants(units) {
    ...
    return `<div class="calc-brief">
      ...
      <div class="calc-or" aria-hidden="true"><span>или</span></div>
  ...
  // Numbered step cards: bracket tag, checkbox, mono title, oversized
  // clipped numeral, side rail with a short status tag. First card is solid.
  const editorial = (items, tag = "") => `<ol class="step-list">${items.map((item, index) => {
    const no = String(index + 1).padStart(2, "0");
    return `<li class="step${index === 0 ? " is-lead" : ""}"><span class="step-tag" aria-hidden="true">[${no}]</span><div class="step-card"><span class="step-check" aria-hidden="true"></span><div class="step-copy"><h3>${esc(item[0])}</h3><p>${esc(item[1])}</p></div><span class="step-no" aria-hidden="true">${no}</span><span class="step-side" aria-hidden="true"><i class="step-open">↗</i><i class="step-x">×</i>${tag ? `<b>${esc(tag)}</b>` : ""}</span></div></li>`;
  }).join("")}</ol>`;
  ...
  function howItWorksSection() {
    return `<section id="how-it-works" class="how-it-works">
      ...
      ${editorial([["Создаёте условия", "Указываете сумму, что покупаете или продаёте, и приглашаете вторую сторону. Обе стороны привязывают свои TON-кошельки."], ["Покупатель оплачивает escrow", "Кошелёк покупателя одной транзакцией создаёт контракт сделки и переводит туда нужную сумму. Деньги не проходят через кошелёк бота."], ["Продавец выполняет условия", "После подтверждения оплаты продавец передаёт товар или оказывает услугу. До этого момента переводить что-либо вне escrow не нужно."], ["Подтверждение или спор", "Если всё хорошо, покупатель подтверждает получение своим кошельком и контракт платит продавцу. Если есть проблема — открывается спор, а арбитр может распределить деньги только между сторонами."]], "шаг")}
  ...
  function profileView() {
    const registryDeals = state.deals
      .filter((deal) => deal.consentEligible && deal.role !== "admin")
      .sort((a, b) => b.id - a.id);
    const registryRows = registryDeals.length
      ? registryDeals.map((deal) => {
          const visible = deal.myConsent === 1;
          return `<div class="registry-manage-row">
            <div><strong>Сделка #${deal.id}</strong><span>${esc(deal.amountTon)} TON · ${visible ? "видна в реестре" : "скрыта"}</span></div>
            <div class="registry-manage-actions">
              <button class="mini-choice ${visible ? "is-active" : ""}" data-registry-consent="yes" data-deal-id="${deal.id}">Показывать</button>
              <button class="mini-choice ${!visible ? "is-active" : ""}" data-registry-consent="no" data-deal-id="${deal.id}">Скрыть</button>
            </div>
          </div>`;
        }).join("")
      : `<p class="hint registry-empty">Завершённых сделок для управления пока нет.</p>`;
    ...
    return `<h1 class="page-title">Профиль</h1><p class="page-intro">TON-адрес используется как адрес покупателя или получателя выплаты. Seed-фраза боту не нужна.</p><form id="wallet-form" class="panel panel-blueprint"><h2>Кошелёк</h2><div class="field"><label for="wallet-address">Ваш TON-адрес</label><input id="wallet-address" name="address" value="${esc(state.user.walletAddress || "")}" placeholder="EQ… или UQ…" required></div><button class="btn btn-primary btn-block">Сохранить адрес</button></form>
      ...
      <section class="registry-manage"><span class="prose-label">ПУБЛИЧНЫЙ РЕЕСТР</span><div class="section-head compact-head"><h3>Управление сделками в реестре</h3></div><p class="registry-manage-copy">Здесь можно в любой момент скрыть или снова показать ваш @username у ранее закрытых сделок.</p>${registryRows}</section>
  ...
  function adminView() {
    if (!state.admin) return `<div class="empty">Нет доступа</div>`;
    const stats = state.admin.stats;
    const diag = state.adminDiagnostics;
    const diagReady = diag && !diag.error && diag.build;
    const runtimeLogs = diagReady ? (diag.runtime.logs || []).join(String.fromCharCode(10)) : "";
    const buildCard = diagReady ? `<section class="admin-diagnostics"><div class="section-head"><h2>Диагностика</h2><button class="text-button" data-admin-refresh="1">Обновить</button></div>
      <p class="page-intro">Здесь доступны текущая версия сборки, runtime-логи и проверочные данные контракта. Полные исторические build/deploy-логи Railway по-прежнему живут в Railway UI/API.</p>
      <div class="admin-grid">
        <article class="admin-info-card"><span class="prose-label">СБОРКА</span><h3>Текущая версия</h3><dl class="kv"><dt>Build ID</dt><dd><button class="copy-value" data-copy="${esc(diag.build)}" data-copy-label="Build ID скопирован">${esc(diag.build)}</button></dd><dt>Запуск</dt><dd>${esc(diag.startupAt)}</dd><dt>Version</dt><dd><a class="text-link" href="${esc(diag.links.version)}" target="_blank" rel="noopener">Открыть /version <span>→</span></a></dd><dt>Health</dt><dd><a class="text-link" href="${esc(diag.links.health)}" target="_blank" rel="noopener">Открыть /health <span>→</span></a></dd></dl></article>
        <article class="admin-info-card"><span class="prose-label">СЕТЬ И КОНТРАКТ</span><h3>Проверка on-chain</h3><dl class="kv"><dt>Сеть</dt><dd>${esc(diag.environment.network)}</dd><dt>Service wallet</dt><dd>${diag.serviceWallet.address ? `<button class="copy-value" data-copy="${esc(diag.serviceWallet.address)}" data-copy-label="Адрес скопирован">${esc(diag.serviceWallet.address)}</button>` : `<span class="hint">не удалось получить</span>`}</dd><dt>Баланс</dt><dd>${diag.serviceWallet.balanceTon ? `${esc(diag.serviceWallet.balanceTon)} TON (${esc(diag.serviceWallet.state || "")})` : esc(diag.serviceWallet.error || "неизвестно")}</dd><dt>Code hash</dt><dd><button class="copy-value" data-copy="${esc(diag.contract.escrowCodeHash)}" data-copy-label="Code hash скопирован">${esc(diag.contract.escrowCodeHash)}</button></dd></dl><div class="link-list" style="margin-top:10px">${diag.serviceWallet.explorers ? `<a href="${esc(diag.serviceWallet.explorers.tonviewer)}" target="_blank" rel="noopener">Tonviewer ↗</a><a href="${esc(diag.serviceWallet.explorers.tonscan)}" target="_blank" rel="noopener">Tonscan ↗</a>` : ""}${diag.contract.verifierUrl ? `<a href="${esc(diag.contract.verifierUrl)}" target="_blank" rel="noopener">TON Verifier ↗</a>` : ""}</div></article>
      </div>
      <details class="admin-note-box"><summary>Что именно хранится здесь <span class="chev" aria-hidden="true">▾</span></summary><div><p>${esc(diag.note)}</p><pre class="admin-build-note">${esc(diag.buildNote || "Build note недоступен")}</pre></div></details>
      <section class="admin-log-box"><div class="section-head compact-head"><h3>Runtime logs</h3><button class="text-button" data-copy="${esc(runtimeLogs)}" data-copy-label="Логи скопированы">Копировать</button></div><pre class="admin-log">${esc(runtimeLogs)}</pre></section>
    </section>` : diag?.error ? `<section class="admin-diagnostics"><div class="section-head"><h2>Диагностика</h2><button class="text-button" data-admin-refresh="1">Повторить</button></div><div class="error-box">${esc(diag.error)}</div></section>` : `<section class="admin-diagnostics"><div class="section-head"><h2>Диагностика</h2><button class="text-button" data-admin-refresh="1">Загрузить</button></div><p class="page-intro">Нажмите, чтобы получить build id, runtime-логи и code hash контракта.</p></section>`;
    return `<h1 class="page-title">Админ-панель</h1><p class="page-intro">Контроль активных сделок, споров и технической диагностики. Blockchain-действия необратимы.</p><div class="stats home-stats admin-stats"><article class="stat stat-primary"><div class="stat-top"><span class="stat-index">01</span><span class="stat-signal"><b></b><b></b><b></b></span></div><strong>${stats.users}</strong><span class="stat-label">Пользова-<br>телей</span><i class="stat-orbit"></i></article><article class="stat stat-solid"><div class="stat-top"><span class="stat-index">02</span><span class="stat-signal"><b></b><b></b><b></b></span></div><strong>${stats.active}</strong><span class="stat-label">Активных<br>сделок</span><i class="stat-grid"></i></article><article class="stat stat-light"><div class="stat-top"><span class="stat-index">03</span><span class="stat-signal"><b></b><b></b><b></b></span></div><strong>${stats.disputed}</strong><span class="stat-label">Открытых<br>споров</span><i class="stat-cross">＋</i></article></div>${buildCard}<div class="section-head"><h2>Последние сделки</h2></div><div class="card-list">${state.admin.recent.map(dealCard).join("") || `<div class="empty">Сделок нет</div>`}</div>`;
  }
  ...
  function consentPanel(deal) {
    if (deal.myConsent !== null && deal.myConsent !== undefined) return "";
    return `<section class="registry-choice">
      <div class="registry-choice-mark">✓</div>
      <div class="registry-choice-copy"><span>СДЕЛКА ЗАКРЫТА</span><h3>Добавить вас в публичный реестр?</h3><p>Покажем ваш @username рядом с этой сделкой. Решение потом можно изменить в Профиле → «Управление сделками в реестре».</p></div>
      <div class="registry-choice-actions"><button class="btn btn-primary" data-consent="yes">Да, добавить</button><button class="text-button" data-consent="no">Нет, скрыть</button></div>
    </section>`;
  }

  function dealProgress(level) {
    return `<section class="deal-progress" aria-label="Ход сделки">
      <div class="deal-progress-head"><span class="prose-label">ХОД СДЕЛКИ</span><strong>${Math.min(level + 1, stages.length)} / ${stages.length}</strong></div>
      <ol>${stages.map((stage, index) => {
        const cls = index < level ? "is-done" : index === level ? "is-current" : "";
        return `<li class="${cls}"><i>${index < level ? "✓" : String(index + 1).padStart(2, "0")}</i><span>${stage[1]}</span></li>`;
      }).join("")}</ol>
    </section>`;
  }

  function dealConditions(deal) {
    return `<section class="deal-conditions"><span class="prose-label">УСЛОВИЯ</span><p class="deal-description">${esc(deal.description)}</p><dl class="deal-party-grid"><div><dt>Покупатель</dt><dd>${deal.buyer?.username ? "@" + esc(deal.buyer.username) : deal.buyer ? `ID ${deal.buyer.id}` : "ожидается"}</dd></div><div><dt>Продавец</dt><dd>${deal.seller?.username ? "@" + esc(deal.seller.username) : deal.seller ? `ID ${deal.seller.id}` : "ожидается"}</dd></div><div><dt>Создана</dt><dd>${formatDate(deal.createdAt)}</dd></div></dl></section>`;
  }

  function dealCalculation(deal) {
    return `<section class="deal-calculation"><div class="deal-calculation-head"><span class="prose-label">РАСЧЁТ ПО СДЕЛКЕ</span><strong>${feePercentText(deal.fees.feePercent)}%</strong></div>${feeTable({ amount: BigInt(deal.amountUnits), serviceFee: toUnits(deal.fees.serviceFeeTon), networkFee: toUnits(deal.fees.networkFeeTon), total: BigInt(deal.fees.totalUnits), feeFromSeller: deal.fees.feeFromSeller, sellerPayout: toUnits(deal.fees.sellerPayoutTon) }, deal.fees.feePercent)}</section>`;
  }

  function dealView(deal) {
    const statusOrder = { draft: 0, awaiting_wallets: 1, setup_pending: 2, deployed: 2, funded: 3, disputed: 3, confirm_pending: 4, completed: 4, cancel_pending: 4, cancelled: 4, resolve_pending: 4, resolved: 4 };
    const level = statusOrder[deal.status] ?? 0;
    const firstStage = ["draft", "awaiting_wallets"].includes(deal.status);
    ...
    const adminActions = state.user.isAdmin && ["deployed", "funded", "disputed"].includes(deal.status) ? `<section class="deal-actions-block"><span class="prose-label">АРБИТР</span><h3>Действия арбитра</h3><div class="button-row"><button class="btn btn-danger" data-action="admin_cancel">Возврат покупателю</button>${["funded", "disputed"].includes(deal.status) ? `<button class="btn" data-action="admin_resolve">Распределить средства</button>` : ""}</div></section>` : "";
    const nextStep = nextStepForDeal(deal);
    const contract = deal.contractAddress ? `<div class="detail-contract">
        <span>Escrow</span>
        <button class="detail-contract-address" data-copy="${esc(deal.contractAddress)}" data-copy-label="Адрес скопирован" title="Нажмите, чтобы скопировать">${esc(deal.contractAddress)}</button>
        <div class="detail-contract-links"><a href="${esc(deal.explorers.tonviewer)}" target="_blank" rel="noopener">Tonviewer ↗</a><a href="${esc(deal.explorers.tonscan)}" target="_blank" rel="noopener">Tonscan ↗</a></div>
      </div>` : `<div class="detail-contract detail-contract-wait"><span>Escrow</span><small>Адрес появится перед оплатой</small></div>`;
    const registryPrompt = deal.consentEligible && deal.role !== "admin" ? consentPanel(deal) : "";

    return `<button class="text-button deal-back" data-route="deals">← Все сделки</button>
      <section class="detail-head detail-head-open">
        <div class="detail-status-row"><span class="badge ${deal.status}">${esc(deal.statusLabel)}</span><span>Сделка #${deal.id}</span></div>
        <h1>${esc(deal.amountTon)} <small>TON</small></h1>
        <p>${deal.role === "buyer" ? "Вы покупатель" : deal.role === "seller" ? "Вы продавец" : "Режим администратора"}</p>
        ${contract}
      </section>
      ${registryPrompt}
      ${dealProgress(level)}
      ...
      ${firstStage ? dealConditions(deal) : ""}
      ${deal.inviteToken ? `<section class="deal-actions-block"><span class="prose-label">ПРИГЛАШЕНИЕ</span><p>Отправьте одноразовую ссылку второй стороне.</p><button class="btn btn-primary btn-block" data-action="share">Поделиться приглашением</button></section>` : ""}
      ${actions ? `<section class="deal-actions-block"><span class="prose-label">ДЕЙСТВИЯ</span><div class="button-row">${actions}</div></section>` : ""}
      ${adminActions}
      ${!firstStage ? dealConditions(deal) : ""}
  ...
  function render() {
    ...
    if (state.route === "admin" && state.user?.isAdmin) void loadAdminDiagnostics(false);
    bindCurrentView();
  ...
  function bindCurrentView() {
    ...
    view.querySelectorAll("[data-admin-refresh]").forEach((item) => item.onclick = () => void loadAdminDiagnostics(true));
    view.querySelectorAll("[data-registry-consent]").forEach((item) => item.onclick = () => setConsent(Boolean(item.dataset.registryConsent === "yes"), Number(item.dataset.dealId)));
  }

  let adminDiagnosticsPromise = null;

  async function loadAdminDiagnostics(force = false) {
    if (!state.user?.isAdmin) return;
    const stale = !state.adminDiagnostics?.loadedAt || Date.now() - state.adminDiagnostics.loadedAt > 30000;
    if (!force && !stale) return;
    if (adminDiagnosticsPromise) return adminDiagnosticsPromise;
    adminDiagnosticsPromise = api("/api/admin/diagnostics").then((data) => {
      state.adminDiagnostics = { ...data, loadedAt: Date.now() };
      if (state.route === "admin") render();
    }).catch((error) => {
      state.adminDiagnostics = { error: error.message, loadedAt: Date.now() };
      if (state.route === "admin") render();
    }).finally(() => { adminDiagnosticsPromise = null; });
    return adminDiagnosticsPromise;
  }
  ...
  async function setConsent(consent, dealId = null) {
    const id = dealId || state.selectedDeal?.id;
    if (!id) return;
    try {
      const data = await api(`/api/deals/${id}/consent`, { method: "POST", body: JSON.stringify({ consent }) });
      if (state.selectedDeal?.id === id) state.selectedDeal = data.deal;
      const index = state.deals.findIndex((item) => item.id === id);
      if (index >= 0) state.deals[index] = { ...state.deals[index], ...data.deal };
