(() => {
  ...
  function homeView() {
    ...
    return `<section class="hero"><p class="eyebrow">TON · SMART CONTRACT ESCROW</p><h1>Безопасные сделки в TON</h1><p>Деньги хранятся в отдельном смарт-контракте сделки и выпускаются только по её правилам.</p><div class="hero-actions"><button class="btn btn-primary" data-route="create">Создать сделку</button><button class="btn btn-secondary" data-route="deals">Мои сделки</button></div></section>
      ...
      <div class="editorial-list"><article><span>01</span><div><strong>Отдельный контракт</strong><p>Каждая сделка получает собственный on-chain адрес.</p></div></article><article><span>02</span><div><strong>Фиксированные стороны</strong><p>Адреса участников и комиссии записаны при создании.</p></div></article><article><span>03</span><div><strong>Проверяемо</strong><p>Баланс и операции доступны в Tonviewer и Tonscan.</p></div></article><article><span>04</span><div><strong>Арбитраж</strong><p>Спор распределяет средства только между заданными сторонами.</p></div></article></div>`;
  }
  ...
  function createView() {
    return `<h1 class="page-title">Новая сделка</h1><p class="page-intro">Укажите условия. После создания отправьте одноразовую ссылку второй стороне.</p><form id="create-form" class="panel panel-blueprint">
      ...
      <div class="field"><label>Кто оплачивает комиссию ${state.config.feePercent}%</label><div class="segmented"><label><input type="radio" name="feePayer" value="buyer" checked><span>Покупатель<br><small>+${state.config.feePercent}% к оплате</small></span></label><label><input type="radio" name="feePayer" value="seller"><span>Продавец<br><small>−${state.config.feePercent}% из выплаты</small></span></label></div></div>
  ...
  function profileView() {
    return `<h1 class="page-title">Профиль</h1><p class="page-intro">TON-адрес используется как адрес покупателя или получателя выплаты. Seed-фраза боту не нужна.</p><form id="wallet-form" class="panel panel-blueprint"><h2>Кошелёк</h2><div class="field"><label for="wallet-address">Ваш TON-адрес</label><input id="wallet-address" name="address" value="${esc(state.user.walletAddress || "")}" placeholder="EQ… или UQ…" required></div><button class="btn btn-primary btn-block">Сохранить адрес</button></form>
      <section class="editorial-section"><p class="section-kicker">TELEGRAM ACCOUNT</p><h2>Аккаунт</h2><dl class="kv"><dt>Имя</dt><dd>${esc([state.user.first_name, state.user.last_name].filter(Boolean).join(" "))}</dd><dt>Username</dt><dd>${state.user.username ? "@" + esc(state.user.username) : "не задан"}</dd><dt>Telegram ID</dt><dd>${state.user.id}</dd><dt>Доступ</dt><dd><span class="access-badge ${state.user.isAdmin ? "admin" : "user"}">${state.user.isAdmin ? "Администратор" : "Пользователь"}</span></dd><dt>Сеть</dt><dd>${esc(state.config.network)}</dd></dl>${state.user.isAdmin ? `<button class="btn btn-primary" data-route="admin">Открыть админ-панель</button>` : ""}</section>
      <section class="support-line"><div><span>НУЖНА ПОМОЩЬ?</span><p>Не сообщайте seed-фразу. При проблеме после оплаты откройте спор.</p></div><a href="https://t.me/${encodeURIComponent(state.config.supportUsername)}">@${esc(state.config.supportUsername)} ↗</a></section>`;
  }
  ...
  function transparencyView() {
    return `<h1 class="page-title">On-chain прозрачность</h1><p class="page-intro">Данные можно самостоятельно проверить в блокчейне TON.</p><div id="transparency-data" class="panel panel-blueprint"><div class="loading-screen" style="min-height:220px"><div class="loader"></div><p>Читаем блокчейн…</p></div></div>
      <div class="editorial-list"><article><span>01</span><div><strong>Депозит не на кошельке бота</strong><p>Средства находятся на отдельном escrow-контракте.</p></div></article><article><span>02</span><div><strong>Получатели неизменяемы</strong><p>Контракт не позволяет арбитру подставить произвольный адрес.</p></div></article></div>
      <section class="editorial-section" id="terms"><p class="section-kicker">ВАЖНО</p><h2>Проверяйте перед переводом</h2><p>Текущая версия — mainnet MVP без независимого аудита. Начинайте с небольших сумм и сверяйте адрес.</p></section><section class="editorial-section" id="privacy"><p class="section-kicker">КОНФИДЕНЦИАЛЬНОСТЬ</p><h2>Без seed-фраз</h2><p>Сервис использует Telegram ID, username, указанный TON-адрес и данные сделки. Seed-фразы не запрашиваются и не сохраняются.</p></section>`;
  }
  ...
  function nextStepForDeal(deal) {
    ...
    if (deal.status === "deployed" && deal.role === "buyer") {
      return { tone: "attention", label: "ШАГ 03 · ОПЛАТА", title: `Внесите ${deal.paymentTon} TON в escrow`, text: "Используйте кнопку ниже: сумма и адрес контракта будут подставлены автоматически.", action: `<button class="btn btn-primary btn-block" data-action="pay">Оплатить ${esc(deal.paymentTon)} TON</button>` };
    }
  ...
  function dealView(deal) {
    ...
    const actions = [
      canPay ? `<button class="btn btn-primary btn-block" data-action="pay">Оплатить ${esc(deal.paymentTon)} TON</button>` : "",
    ...
    return `<button class="text-button" data-route="deals">← Все сделки</button><section class="detail-head"><span class="badge ${deal.status}">${esc(deal.statusLabel)}</span><h1>${esc(deal.amountTon)} TON</h1><p>Сделка #${deal.id} · ${deal.role === "buyer" ? "вы покупатель" : deal.role === "seller" ? "вы продавец" : "режим администратора"}</p></section>
      ...
      <section class="editorial-section deal-terms"><p class="section-kicker">УСЛОВИЯ</p><h2>${esc(deal.description)}</h2><dl class="kv"><dt>Покупатель</dt><dd>${deal.buyer?.username ? "@" + esc(deal.buyer.username) : deal.buyer ? `ID ${deal.buyer.id}` : "ожидается"}</dd><dt>Продавец</dt><dd>${deal.seller?.username ? "@" + esc(deal.seller.username) : deal.seller ? `ID ${deal.seller.id}` : "ожидается"}</dd><dt>Комиссия</dt><dd>${deal.feePayer === "buyer" ? `покупатель · к оплате ${esc(deal.paymentTon)} TON` : `продавец · получит ${esc(deal.sellerReceivesTon)} TON`}</dd><dt>Создана</dt><dd>${formatDate(deal.createdAt)}</dd></dl></section>
      ...
      <section class="editorial-section"><p class="section-kicker">ПРОГРЕСС</p><h2>Ход сделки</h2><ol class="timeline">${stages.map((stage, index) => `<li class="${level >= index ? "done" : ""}">${stage[1]}</li>`).join("")}</ol></section>
  ...
  async function createDeal(event) {
    ...
    try {
      const data = await api("/api/deals", { method: "POST", body: JSON.stringify({ role: form.get("role"), feePayer: form.get("feePayer"), counterpartyUsername: form.get("counterparty"), amountTon: String(form.get("amount")).replace(",", "."), description: form.get("description") }) });
      await refresh();
  ...
  async function payDeal() {
    ...
    if (!deal?.contractAddress) throw new Error("Escrow-адрес ещё не готов");
    const invoice = `https://app.tonkeeper.com/transfer/${encodeURIComponent(deal.contractAddress)}?amount=${encodeURIComponent(deal.paymentUnits)}`;
    if (tg?.openLink) tg.openLink(invoice); else window.location.href = invoice;
  }
  ...
  function dealAction(action) {
    ...
    if (action === "share") {
      const url = deal.inviteUrl || `https://t.me/${state.config.botUsername}?startapp=join_${deal.inviteToken}`;
      return showInvite(url);
    }
    if (action === "confirm") return showModal("Подтвердить получение?", deal.feePayer === "buyer" ? "После подтверждения продавец получит всю сумму сделки, а комиссия покупателя будет отправлена платформе." : "После подтверждения продавец получит сумму сделки за вычетом комиссии платформы.", "Да, всё получено", () => postAction("confirm"));
  ...
  async function handleDealParam() {
    const id = Number(new URLSearchParams(location.search).get("deal"));
    if (Number.isSafeInteger(id) && id > 0) await openDeal(id);
  }
