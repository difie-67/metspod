(() => {
  ...
  const JOURNEY = [
    ...
    { n: "03", glyph: "◎", title: "Оплатите", text: "TON поступят в отдельный смарт-контракт." },
  ...
  setInterval(() => {
    if (document.hidden || !document.querySelector(".jmap") || Date.now() - journeyTouched < 7000) return;
  ...
  function siteFooter() {
    ...
    const legal = (key, label) => LEGAL_LINKS[key] ? `<a href="${esc(LEGAL_LINKS[key])}" target="_blank" rel="noopener">${label}</a>` : `<span class="soon">${label}<small>скоро</small></span>`;
    return `<div class="footer-inner">
      <div class="footer-brand"><span class="brand-mark">O</span><div><b>OBRA GUARANT</b><small>Безопасные сделки в TON</small></div></div>
      <div class="footer-cols">
        <div class="footer-col"><h4>Поддержка</h4><a data-tg href="https://t.me/${esc(support)}">@${esc(support)}</a><p>Споры, вопросы по сделкам и оплате</p></div>
        <div class="footer-col"><h4>Проект</h4>${COMMUNITY.map((item) => `<a data-tg href="https://t.me/${esc(item.user)}"><span>${esc(item.label)}</span> @${esc(item.user)}</a>`).join("")}</div>
        <div class="footer-col"><h4>Карта приложения</h4><button data-route="home">Главная</button><button data-route="deals">Мои сделки</button><button data-route="create">Создать сделку</button><button data-route="transparency">On-chain прозрачность</button><button data-route="profile">Профиль и кошелёк</button></div>
        <div class="footer-col"><h4>Юридическая информация</h4>${legal("privacy", "Политика конфиденциальности")}${legal("terms", "Пользовательское соглашение")}<p>Документы готовятся к публикации.</p></div>
      </div>
      <p class="footer-copy">© ${years} OBRA GUARANT. Все права защищены.<br>Escrow-сервис на TON · сеть ${esc(state.config?.network || "mainnet")}</p>
    </div>`;
  }
