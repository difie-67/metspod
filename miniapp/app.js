(() => {
  ...
  function quickPricingSection() {
    return `<section class="pricing-section" id="pricing">
      ...
      <p class="pricing-lead">Процент зависит только от суммы сделки: чем выше сумма, тем ниже ставка. Отдельно к каждой сделке добавляется фиксированный сетевой сбор <b>${esc(state.config.networkFeeTon)} TON</b> на операции смарт-контракта.</p>
      ...
      <div class="pricing-explain">
        ...
        <div><span class="prose-label">СЕТЬ TON</span><h3>Почему ещё есть ${esc(state.config.networkFeeTon)} TON</h3><p>Это бюджет на on-chain действия: развёртывание escrow и последующие транзакции. Неиспользованный газ возвращается служебному кошельку.</p><a class="text-link" href="${esc(state.config.gasInfoUrl || "https://docs.ton.org/contracts/techniques/gas")}" target="_blank" rel="noopener">Как устроен gas в TON <span>→</span></a></div>
      </div>
