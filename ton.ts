...
import { Escrow, storeConfirm } from "../contracts/wrappers/Escrow";
...
export async function getEscrowDeploymentData(params: EscrowParams) {
  const escrow = await makeEscrow(params);
  if (!escrow.init) throw new Error("Escrow init data is unavailable");
  const stateInitCell = beginCell().store(storeStateInit(escrow.init)).endCell();
  return {
...
export function makeConfirmPayload(dealId: number): string {
  return beginCell()
    .store(storeConfirm({ $$type: "Confirm", dealId: BigInt(dealId) }))
    .endCell()
    .toBoc()
    .toString("base64");
}
