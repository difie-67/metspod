...
import { getDealOnChainState, isContractActive, parseAddress } from "./ton";
import { bot } from "./bot";
...
async function onFunded(deal: DealRow) {
  ...
  if (deal.buyer_tg_id) {
    ...
    await bot.api.sendMessage(
      deal.buyer_tg_id,
      `Оплата ${amount(deal)} по сделке #${deal.deal_id} получена escrow-контрактом.\n` +
  ...
  if (deal.seller_tg_id) {
    ...
    await bot.api.sendMessage(
      deal.seller_tg_id,
      `Оплата ${amount(deal)} по сделке #${deal.deal_id} получена. Можно передавать товар или оказывать услугу.\n` +
      "После подтверждения покупателем вы получите 98% суммы.",
      { reply_markup: keyboard }
    ).catch(() => undefined);
  }
}
  ...
  for (const deal of deals) {
    ...
    try {
      ...
      if (state === 0) {
        ...
        await notify(
          [deal.buyer_tg_id, deal.seller_tg_id, ...config.arbiterTgIds],
          `✅ Сделка #${deal.deal_id} завершена: 98% отправлено продавцу, 2% — платформе.`
        );
