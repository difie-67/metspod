...
import { config, dealLimits, feeThresholdUnits, networkFeeUnits } from "./config";
import {
  cancelDraftDeal,
  claimDealSetup,
...
function feeLines(deal: DealRow): string {
  ...
  const commissionNote = fees.feeBps === 0
    ? `Комиссия сервиса: 0 TON (0% для сделок до ${config.feeThresholdTon} TON включительно)`
    : fees.feeFromSeller
      ? `Комиссия сервиса (${fees.feeBps / 100}%, удерживается с продавца): ${formatTonAmount(fees.serviceFee)} TON`
      : `Комиссия сервиса (${fees.feeBps / 100}%, сверху от покупателя): ${formatTonAmount(fees.serviceFee)} TON`;
  return [
...
export async function tryAdvanceDeal(dealId: number) {
  ...
  try {
    ...
    const text =
      ...
      `Покупатель должен перевести ровно ${formatDealTotal(deal)} на адрес:\n${monospace(formattedContract)}\n\n` +
      `${feeLines(deal)}\n\n` +
    ...
    if (deal.buyer_tg_id && tonPaymentUrl(paymentDeal)) {
      const surchargeNote = fees.feeBps === 0
        ? `(сумма сделки + сетевой сбор; комиссия сервиса 0% для сумм до ${config.feeThresholdTon} TON включительно)`
        : fees.feeFromSeller
          ? "(сумма сделки + сетевой сбор; комиссия сервиса удержится из выплаты продавцу)"
          : "(сумма сделки + комиссия сервиса + сетевой сбор)";
      await bot.api.sendMessage(
        deal.buyer_tg_id,
        `💳 Оплата сделки #${dealId}\n\nСумма к оплате: ${formatDealTotal(deal)}\n${surchargeNote}\n` +
...
async function showTransparency(ctx: Context) {
  ...
  await ctx.reply(
    ...
    `Сеть: ${config.tonNetwork}\n` +
    `Комиссия сервиса: 0% до ${config.feeThresholdTon} TON включительно; свыше ${config.feeThresholdTon} TON — ${config.feeBps / 100}%. Сетевой сбор: ${formatTonAmount(networkFeeUnits)} TON.\n` +
...
bot.callbackQuery(/^admin_resolve_prepare:(\d+):(0|50|100)$/, async (ctx) => {
  ...
  await ctx.answerCallbackQuery();
  await ctx.editMessageText(
    `Подтвердить решение по сделке #${dealId}: продавцу ${sellerPercent}%, покупателю ${100 - sellerPercent}% суммы сделки (${depositFor(deal).feeBps === 0 ? "комиссия сервиса 0%" : "комиссия сервиса удерживается отдельно"})?`,
...
async function adminAction(ctx: Context, action: "cancel" | "resolve") {
  ...
  if (action === "cancel") {
    ...
    await ctx.reply(`Подтвердить решение по сделке #${dealId}: продавцу ${sellerPercent}%, покупателю ${100 - sellerPercent}% суммы сделки (${depositFor(deal).feeBps === 0 ? "комиссия сервиса 0%" : "комиссия сервиса удерживается отдельно"})?`, {
      reply_markup: new InlineKeyboard().text("Да, отправить Resolve", `admin_resolve_yes:${dealId}:${sellerPercent}`).text("Нет", `view_deal:${dealId}`),
    });
  }
}
...
bot.on("message:text", async (ctx, next) => {
  ...
  if (state.step === "counterparty") {
    ...
    state.step = "amount";
    wizards.set(ctx.from.id, state);
    await ctx.reply(`Введите сумму сделки в TON от ${config.minDealTon} до ${config.maxDealTon}, например 2.5.\nДо ${config.feeThresholdTon} TON включительно комиссия сервиса 0%; свыше ${config.feeThresholdTon} TON — ${config.feeBps / 100}%. Сетевой сбор ~${formatTonAmount(networkFeeUnits)} TON.`);
    return;
  }
  if (state.step === "amount") {
    try {
      ...
      if (amount <= feeThresholdUnits) {
        state.feeFromSeller = false;
        state.step = "description";
        wizards.set(ctx.from.id, state);
        const feeFree = feeBreakdown(amount, 0, networkFeeUnits, false);
        await ctx.reply(
          `Комиссия сервиса — 0%: сумма не выше ${config.feeThresholdTon} TON.\n` +
          `Покупатель заплатит ${formatTonAmount(feeFree.total)} TON (сумма сделки + сетевой сбор ${formatTonAmount(networkFeeUnits)} TON).\n\nОпишите товар или услугу и условия передачи:`
        );
      } else {
        state.step = "fee_payer";
        wizards.set(ctx.from.id, state);
        const feePercent = config.feeBps / 100;
        const buyerOnTop = feeBreakdown(amount, config.feeBps, networkFeeUnits, false);
        const sellerDeducted = feeBreakdown(amount, config.feeBps, networkFeeUnits, true);
        await ctx.reply(
          `Кто платит комиссию сервиса (${feePercent}%)?\n\n` +
