...
function tonPaymentUrl(deal: DealRow): string | null {
  ...
  return `ton://transfer/${encodeURIComponent(deal.contract_address)}?amount=${encodeURIComponent(deal.amount_units)}`;
}
...
function myTonWalletPaymentUrl(deal: DealRow): string | null {
  ...
  return `https://my.tt/transfer/${encodeURIComponent(deal.contract_address)}?amount=${encodeURIComponent(deal.amount_units)}`;
}
...
export async function tryAdvanceDeal(dealId: number) {
  ...
  try {
    ...
    const deployParams = { dealId, buyer, seller, amountUnits: BigInt(deal.amount_units) };
    ...
    const contractAddress = await deployEscrow(deployParams);
    ...
    const text =
      ...
      `Покупатель должен перевести ровно ${formatDealAmount(deal)} на адрес:\n${monospace(formattedContract)}\n\n` +
