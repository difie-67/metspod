...
function feeLines(deal: DealRow): string {
  ...
  return [
    `Продавец получит: ${formatTonAmount(fees.amount)} TON`,
    `Комиссия сервиса (${fees.feeBps / 100}%): ${formatTonAmount(fees.serviceFee)} TON`,
...
type WizardStep = "wallet" | "role" | "counterparty" | "amount" | "description";
...
export async function tryAdvanceDeal(dealId: number) {
  ...
  try {
    ...
    const deployParams = {
      ...
      feeBps: fees.feeBps, networkFeeUnits: fees.networkFee,
    };
    ...
    if (deal.buyer_tg_id && tonPaymentUrl(paymentDeal)) {
      await bot.api.sendMessage(
        deal.buyer_tg_id,
        `💳 Оплата сделки #${dealId}\n\nСумма к оплате: ${formatDealTotal(deal)}\n(сумма сделки ${formatDealAmount(deal)} + комиссия сервиса и сетевой сбор)\n` +
...
bot.on("message:text", async (ctx, next) => {
  ...
  if (state.step === "counterparty") {
    ...
    await ctx.reply(`Введите сумму сделки в TON (сколько получит продавец) от ${config.minDealTon} до ${config.maxDealTon}, например 2.5.\nСверху покупатель оплатит ${config.feeBps / 100}% комиссии сервиса и сетевой сбор ${formatTonAmount(networkFeeUnits)} TON:`);
    return;
  }
  ...
  if (state.step === "description") {
    ...
    const deal = createDeal({ creatorTgId: ctx.from.id, creatorRole: state.role!, counterpartyUsername: state.counterpartyUsername ?? null, amountUnits: state.amountUnits!, description: text });
    wizards.delete(ctx.from.id);
    let username: string;
    try {
