...
/** Exact deposit the buyer must send: amount + service fee + network fee. */
...
function dealSummary(deal: DealRow): string {
  return [
    ...
    `К оплате покупателем: ${formatDealTotal(deal)} (с комиссиями)`,
...
export async function tryAdvanceDeal(dealId: number) {
  ...
  try {
    ...
    if (deal.buyer_tg_id && tonPaymentUrl(paymentDeal)) {
      const surchargeNote = fees.feeFromSeller
...
async function showTransparency(ctx: Context) {
  ...
  await ctx.reply(
    ...
    `Сеть: ${config.tonNetwork}\n` +
    `Комиссия сервиса: ${config.feeBps / 100}% + сетевой сбор ${formatTonAmount(networkFeeUnits)} TON (оплачивает покупатель сверху)\n` +
...
bot.callbackQuery(/^admin_resolve_prepare:(\d+):(0|50|100)$/, async (ctx) => {
  ...
  await ctx.answerCallbackQuery();
  await ctx.editMessageText(
    `Подтвердить решение по сделке #${dealId}: продавцу ${sellerPercent}%, покупателю ${100 - sellerPercent}% суммы сделки (комиссия сервиса удерживается отдельно)?`,
...
async function adminAction(ctx: Context, action: "cancel" | "resolve") {
  ...
  if (action === "cancel") {
    ...
    await ctx.reply(`Подтвердить решение по сделке #${dealId}: продавцу ${sellerPercent}%, покупателю ${100 - sellerPercent}% суммы сделки (комиссия сервиса удерживается отдельно)?`, {
      reply_markup: new InlineKeyboard().text("Да, отправить Resolve", `admin_resolve_yes:${dealId}:${sellerPercent}`).text("Нет", `view_deal:${dealId}`),
    });
  }
}
...
bot.on("message:text", async (ctx, next) => {
  ...
  if (state.step === "counterparty") {
    ...
    state.step = "amount";
    wizards.set(ctx.from.id, state);
    await ctx.reply(`Введите сумму сделки в TON от ${config.minDealTon} до ${config.maxDealTon}, например 2.5.\nКомиссия сервиса ${config.feeBps / 100}% и сетевой сбор ~${formatTonAmount(networkFeeUnits)} TON — на следующем шаге выберете, кто платит комиссию:`);
    return;
  }
