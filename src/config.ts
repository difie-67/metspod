...
export const config = {
  ...
  // Service fee is charged only for deals strictly above this amount.
  // At the threshold itself (7 TON by default) the service fee is still 0%.
  feeThresholdTon: process.env.FEE_THRESHOLD_TON || "7",
  // Real TON deploy/action gas is roughly 0.01-0.03 TON; these keep a safety
  // margin without the old, much larger padding. Any unused balance is
  // returned to the platform address by the escrow contract after settlement.
...
/** Amount threshold (nanotons): service fee applies only when amount > threshold. */
export const feeThresholdUnits: bigint = parseTonAmount(config.feeThresholdTon);
...
if (feeThresholdUnits < 0n) {
  throw new Error("FEE_THRESHOLD_TON cannot be negative");
}
