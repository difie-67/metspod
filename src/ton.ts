...
import { dealFees } from "./fees";
import { networkFeeUnits } from "./config";
...
async function makeEscrow(params: EscrowParams) {
  ...
  return Escrow.fromInit(
    ...
    BigInt(params.feeBps ?? config.feeBps), params.networkFeeUnits ?? networkFeeUnits
  );
}

/** Exact deposit (nanotons) the buyer must send for a stored deal. */
export function depositFor(deal: { amount_units: string; fee_bps: number | null; network_fee_units: string | null }) {
  return dealFees(deal, { feeBps: config.feeBps, networkFee: networkFeeUnits });
}
