...
const BUYER_PAYS_FEE_FLAG = 32768;

async function makeEscrow(params: { dealId: number; buyer: Address; seller: Address; amountUnits: bigint; feePayer: "buyer" | "seller" }) {
  const arbiter = await getArbiterWallet();
  const encodedFeeBps = config.feeBps + (params.feePayer === "buyer" ? BUYER_PAYS_FEE_FLAG : 0);
  return Escrow.fromInit(
    BigInt(params.dealId), params.buyer, params.seller, arbiter.address,
    Address.parse(config.platformAddress), params.amountUnits, BigInt(encodedFeeBps)
  );
}
...
export async function computeEscrowAddress(params: { dealId: number; buyer: Address; seller: Address; amountUnits: bigint; feePayer: "buyer" | "seller" }) {
...
export async function deployEscrow(
  params: { dealId: number; buyer: Address; seller: Address; amountUnits: bigint; feePayer: "buyer" | "seller" },
