(() => {
  const UI_BUILD_ID = "2026-09-24_00-15_dealux7";
  ...
  function siteMapMarkup(className = "site-map", includePrimary = true) {
    return `<nav class="${className}" aria-label="Карта сайта">
      ${includePrimary ? `<button data-route="home">Главная</button><button data-route="deals">Сделки</button><button data-route="create">Создать</button><button data-route="transparency">On-chain</button><button data-route="profile">Профиль</button><span aria-hidden="true"></span>` : ""}
  ...
  function homeView() {
    ...
    return `<section class="hero-open"><img class="hero-mark" src="/miniapp/logo-mark.png" alt="" aria-hidden="true" width="56" height="56" /><p class="hero-eyebrow">TON · SMART CONTRACT ESCROW</p><h1>Безопасные<br>сделки в TON</h1><p>Деньги хранятся в отдельном смарт-контракте сделки и выпускаются только по её правилам.</p><div class="hero-actions"><button class="btn btn-primary" data-route="create">Создать сделку</button><button class="btn btn-secondary" data-anchor="trust">Боюсь, что скам</button></div>${siteMapMarkup("hero-site-map", false)}</section>
      <div class="stats home-stats">
        <button class="stat stat-primary stat-clickable ${active.length ? "stat-hot" : ""}" data-route="deals" data-no="01"><div class="stat-top"><span class="stat-signal"><b></b><b></b><b></b></span></div><strong data-count="${active.length}">${active.length}</strong><span class="stat-label">Активные<br>сделки</span><i class="stat-orbit"></i></button>
        <article class="stat stat-solid" data-no="02"><div class="stat-top"><span class="stat-signal"><b></b><b></b><b></b></span></div><strong data-count="${completed.length}">${completed.length}</strong><span class="stat-label">Завершено<br>успешно</span><i class="stat-grid"></i></article>
        <article class="stat stat-light" data-no="03"><div class="stat-top"><span class="stat-signal"><b></b><b></b><b></b></span></div><strong>0.6–2%</strong><span class="stat-label">Комиссия платформы<br>по сетке</span><i class="stat-cross">＋</i></article>
      </div>
      ${trustSection()}
      ${sectionDivider()}
      ${howItWorksSection()}
  ...
  function adminView() {
    ...
    const d = state.admin.diagnostics || {};
    const logs = Array.isArray(d.logs) ? d.logs : [];
    const contract = d.contract || {};
    const wallet = d.serviceWallet || { explorers: {} };
    return `<h1 class="page-title">Админ-панель</h1><p class="page-intro">Контроль активных сделок и споров, плюс мобильная диагностика: билд, рантайм-логи и отпечатки контракта.</p><div class="stats home-stats admin-stats"><article class="stat stat-primary" data-no="01"><div class="stat-top"><span class="stat-signal"><b></b><b></b><b></b></span></div><strong>${stats.users}</strong><span class="stat-label">Пользова-<br>телей</span><i class="stat-orbit"></i></article><article class="stat stat-solid" data-no="02"><div class="stat-top"><span class="stat-signal"><b></b><b></b><b></b></span></div><strong>${stats.active}</strong><span class="stat-label">Активных<br>сделок</span><i class="stat-grid"></i></article><article class="stat stat-light" data-no="03"><div class="stat-top"><span class="stat-signal"><b></b><b></b><b></b></span></div><strong>${stats.disputed}</strong><span class="stat-label">Открытых<br>споров</span><i class="stat-cross">＋</i></article></div>
      <section class="plain admin-block"><span class="prose-label">DEPLOY / BUILD</span><h3>Текущий билд</h3><dl class="kv kv-plain"><dt>Build ID</dt><dd><button class="copy-value copy-button" data-copy="${esc(d.buildId || '')}">${esc(d.buildId || '—')}</button></dd><dt>Запущен</dt><dd>${d.startedAt ? formatDate(d.startedAt) : '—'}</dd><dt>Uptime</dt><dd>${Number.isFinite(Number(d.uptimeSec)) ? `${Math.floor(Number(d.uptimeSec) / 60)} мин` : '—'}</dd><dt>Network</dt><dd>${esc(d.network || '—')}</dd><dt>Mini App</dt><dd>${d.miniAppUrl ? `<a class="text-link" href="${esc(d.miniAppUrl)}" target="_blank" rel="noopener">Открыть домен <span>→</span></a>` : '—'}</dd></dl></section>
      <section class="plain admin-block"><span class="prose-label">VERIFY</span><h3>Верификация контракта и исходников</h3><dl class="kv kv-plain"><dt>Code hash</dt><dd><button class="copy-value copy-button" data-copy="${esc(contract.codeHash || '')}">${esc(contract.codeHash || '—')}</button></dd><dt>escrow.tact</dt><dd><button class="copy-value copy-button" data-copy="${esc(contract.sourceSha256 || '')}">${esc(contract.sourceSha256 || '—')}</button></dd><dt>src/ton.ts</dt><dd><button class="copy-value copy-button" data-copy="${esc(contract.tonModuleSha256 || '')}">${esc(contract.tonModuleSha256 || '—')}</button></dd><dt>package-lock</dt><dd><button class="copy-value copy-button" data-copy="${esc(contract.lockfileSha256 || '')}">${esc(contract.lockfileSha256 || '—')}</button></dd><dt>Verifier</dt><dd>${contract.verifierUrl ? `<a class="text-link" href="${esc(contract.verifierUrl)}" target="_blank" rel="noopener">TON Verifier <span>→</span></a>` : 'ещё не задан'}</dd></dl></section>
      <section class="plain admin-block"><span class="prose-label">SERVICE WALLET</span><h3>Арбитражный кошелёк</h3><dl class="kv kv-plain"><dt>Адрес</dt><dd><button class="copy-value copy-button" data-copy="${esc(wallet.address || '')}">${esc(wallet.address || '—')}</button></dd><dt>Баланс</dt><dd>${wallet.balanceTon ? `${esc(wallet.balanceTon)} TON` : '—'}</dd><dt>Состояние</dt><dd>${esc(wallet.state || '—')}</dd><dt>Explorer</dt><dd>${wallet.explorers?.tonviewer ? `<a class="text-link" href="${esc(wallet.explorers.tonviewer)}" target="_blank" rel="noopener">Tonviewer <span>→</span></a> <a class="text-link" href="${esc(wallet.explorers.tonscan)}" target="_blank" rel="noopener">Tonscan <span>→</span></a>` : '—'}</dd></dl></section>
      <section class="plain admin-block"><span class="prose-label">RUNTIME LOGS</span><h3>Последние события сервиса</h3><div class="admin-log-list">${logs.length ? logs.map((entry) => `<div class="admin-log admin-log-${esc(entry.level || 'info')}"><div class="admin-log-top"><span>${esc((entry.level || 'info').toUpperCase())}</span><time>${esc(String(entry.ts || '').replace('T',' ').replace('Z',''))}</time></div><button class="copy-value copy-button" data-copy="${esc(`[${entry.level}] ${entry.ts} ${entry.message}`)}">${esc(entry.message || '')}</button></div>`).join('') : `<div class="empty">Логи пока пусты</div>`}</div></section>
      <div class="section-head"><h2>Последние сделки</h2></div><div class="card-list">${state.admin.recent.map(dealCard).join("") || `<div class="empty">Сделок нет</div>`}</div>`;
  }
  ...
  function dealProgress(level) {
    return `<section class="deal-progress" aria-label="Ход сделки">
      <div class="deal-progress-head"><span class="prose-label">ХОД СДЕЛКИ</span><strong class="mono-digits">${Math.min(level + 1, stages.length)} / ${stages.length}</strong></div>
      <ol>${stages.map((stage, index) => {
        const cls = index < level ? "is-done" : index === level ? "is-current" : "";
        return `<li class="${cls}" data-no="${String(index + 1).padStart(2, "0")}"><i>${index < level ? "✓" : String(index + 1).padStart(2, "0")}</i><span>${stage[1]}</span></li>`;
      }).join("")}</ol>
    </section>`;
  }
  ...
  function dealView(deal) {
    ...
    return `<button class="text-button deal-back" data-route="deals">← Все сделки</button>
      <section class="detail-head detail-head-open">
        <div class="detail-status-row"><span class="badge ${deal.status}">${esc(deal.statusLabel)}</span><span class="deal-no mono-digits">Сделка #${deal.id}</span></div>
        ...
        ${contract}
        <div class="detail-ghost-amount" aria-hidden="true">${esc(deal.amountTon)}</div>
      </section>
      ${registryPrompt}
      ${dealProgress(level)}
  ...
  function render() {
    ...
    updateTabIndicator();
    walletPill.innerHTML = state.user.walletAddress ? `${esc(short(state.user.walletAddress))}` : `<span class="wallet-pill-icon" aria-hidden="true"></span><span>Добавить кошелёк</span>`;
    walletPill.classList.toggle("is-set", !!state.user.walletAddress);
    walletPill.classList.toggle("has-icon", !state.user.walletAddress);
    let html = null;
  ...
  async function saveWallet(event) {
    ...
    try {
      ...
      walletPill.innerHTML = esc(short(data.walletAddress));
      walletPill.classList.add("is-set");
      walletPill.classList.remove("has-icon");
