(() => {
  ...
  function effectiveFeePercent(amountUnits) {
    const threshold = toUnits(state.config?.feeThresholdTon ?? "7") ?? 7000000000n;
    return amountUnits > threshold ? Number(state.config?.feePercent || 0) : 0;
  }
  ...
  function feeTable(rows, feePercent) {
    const threshold = state.config?.feeThresholdTon ?? "7";
    const commissionLabel = feePercent > 0
      ? (rows.feeFromSeller ? `Комиссия сервиса ${feePercent}% (с продавца)` : `Комиссия сервиса ${feePercent}% (сверху)`)
      : `Комиссия сервиса (0% до ${threshold} TON включительно)`;
  ...
  function sectionDivider(label = "") {
    return `<div class="section-divider section-divider-word" aria-hidden="true"><span class="divider-word">${esc(label)}</span></div>`;
  }
  function journeyMap() {
  ...
  function homeView() {
    ...
    return `<section class="hero-open"><img class="hero-mark" src="/miniapp/logo-mark.png" alt="" aria-hidden="true" width="56" height="56" /><p class="hero-eyebrow">TON · SMART CONTRACT ESCROW</p><h1>Безопасные<br>сделки в TON</h1><p>Деньги хранятся в отдельном смарт-контракте сделки и выпускаются только по её правилам.</p><div class="hero-actions"><button class="btn btn-primary" data-route="create">Создать сделку</button><button class="btn btn-secondary" data-anchor="trust">А если скам?</button></div></section>
      <div class="stats home-stats">
        ...
        <article class="stat stat-light"><div class="stat-top"><span class="stat-index">03</span><span class="stat-signal"><b></b><b></b><b></b></span></div><strong>${state.config.feePercent}%</strong><span class="stat-label">Только выше<br>${esc(state.config.feeThresholdTon)} TON</span><i class="stat-cross">＋</i></article>
      </div>
      ${trustSection()}
      ${sectionDivider("scenario")}
      ${journeyMap()}
      ${sectionDivider("deals")}
  ...
  function nextStepForDeal(deal) {
    ...
    if (deal.status === "deployed" && deal.role === "buyer") {
      const breakdown = Number(deal.fees.feePercent) === 0
        ? `Сумма сделки ${deal.amountTon} TON + сетевой сбор ${deal.fees.networkFeeTon} TON. Комиссия сервиса — 0%.`
        : deal.fees.feeFromSeller
    ...
    if (deal.status === "cancelled") return { tone: "calm", label: "СДЕЛКА ОТМЕНЕНА", title: "Сделка закрыта", text: Number(deal.fees.feePercent) === 0 ? "Если оплата успела поступить в контракт, сумма сделки возвращена покупателю; сетевой сбор остаётся на газ." : "Если оплата успела поступить в контракт, покупателю возвращены сумма сделки и комиссия сервиса (сетевой сбор остаётся на газ)." };
  ...
  function bindCurrentView() {
    ...
    if (amountInput) {
      const update = () => {
        ...
        if (feePayerField) feePayerField.hidden = feePercent === 0;
        const feeFromSeller = feePercent > 0 && view.querySelector('input[name="feePayer"]:checked')?.value === "seller";
        target.innerHTML = units && units > 0n ? feeTable(feeRows(units, feePercent, state.config.networkFeeTon, feeFromSeller), feePercent) : `<p class="fee-empty">Введите сумму — до ${esc(state.config.feeThresholdTon)} TON включительно комиссия сервиса 0%.</p>`;
      };
  ...
  function dealAction(action) {
    ...
    if (action === "confirm") {
      const feeNote = Number(deal.fees.feePercent) === 0
        ? "Комиссия сервиса для этой сделки — 0%."
        : `Комиссия сервиса ${deal.fees.serviceFeeTon} TON будет удержана по условиям сделки.`;
    ...
    if (action === "admin_cancel") {
      const refundText = Number(deal.fees.feePercent) === 0
        ? "Покупателю возвращается сумма сделки. Сетевой сбор/газ учитывается контрактом отдельно. Blockchain-действие необратимо."
        : "Покупателю возвращаются сумма сделки и сервисная комиссия; сетевой сбор/газ учитывается контрактом отдельно. Blockchain-действие необратимо.";
      return showModal("Вернуть покупателю?", refundText, "Отправить возврат", () => postAction("admin_cancel"), true);
    }
    if (action === "admin_resolve") {
      modal.hidden = false;
      const disputeFeeNote = Number(deal.fees.feePercent) === 0 ? "Комиссия сервиса для этой сделки — 0%." : "Комиссия сервиса удерживается отдельно по условиям сделки.";
