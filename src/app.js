(() => {
  ...
  function priceTierCards() {
    return feeSchedule().map((tier, index) => `<article class="price-card" data-tier-key="${esc(tier.key)}"><div class="price-card-top"><span class="price-card-index">0${index + 1}</span><span class="price-card-range">${esc(tier.shortLabel)}</span></div><strong>${esc(tier.feePercentText)}%</strong><p>${esc(tier.title)}</p><i class="price-card-glow" aria-hidden="true"></i></article>`).join("");
  }
  function calculatorVariants(units) {
    ...
    const sellerDeduction = units - sellerRows.sellerPayout;
    return `<div class="fee-tier-note"><div class="calc-summary-strip"><span class="calc-summary-chip">Ставка ${esc(tier.feePercentText)}%</span><span class="calc-summary-chip ghost">Диапазон ${esc(tier.shortLabel)}</span><span class="calc-summary-chip ghost">Газ ${esc(state.config.networkFeeTon)} TON</span></div><p>Для введённой суммы автоматически выбрана подходящая ставка платформы и фиксированный сетевой сбор.</p></div>
      <div class="calc-variants">
        <article class="calc-variant calc-variant-primary">
          <div class="calc-variant-top"><span class="variant-kicker">Вариант 1</span><h4>Комиссию сверху платит покупатель</h4></div>
          <div class="calc-hero"><strong>${esc(fromUnits(buyerRows.total))} TON</strong><span class="calc-chip">Переплата: ${esc(fromUnits(buyerExtra))} TON</span></div>
          <p class="calc-subline">Продавец получит всю сумму сделки: ${esc(fromUnits(units))} TON.</p>
          ${feeTable(buyerRows, feePercent)}
        </article>
        <article class="calc-variant">
          <div class="calc-variant-top"><span class="variant-kicker">Вариант 2</span><h4>Комиссия удерживается из выплаты продавцу</h4></div>
          <div class="calc-hero"><strong>${esc(fromUnits(sellerRows.total))} TON</strong><span class="calc-chip">Удержание продавца: ${esc(fromUnits(sellerDeduction))} TON</span></div>
          <p class="calc-subline">Покупатель платит только сумму сделки и газ, а продавец получает меньше на размер комиссии.</p>
          ${feeTable(sellerRows, feePercent)}
        </article>
      </div>`;
  }
  function quickPricingSection() {
    return `<section class="pricing-section" id="pricing"><div class="section-head"><h2>Комиссия и калькулятор</h2></div>
      <div class="pricing-overview panel panel-blueprint"><div class="pricing-overview-copy"><p class="page-intro">У сервиса есть две отдельные части расходов: <b>комиссия платформы</b> на поддержку работы сервиса и <b>фиксированный сетевой сбор ${esc(state.config.networkFeeTon)} TON</b>, который уходит не нам, а покрывает on-chain действия в TON.</p></div><div class="pricing-overview-stats"><div class="pricing-pill"><span>Ставки</span><strong>0.6–2%</strong></div><div class="pricing-pill"><span>Сетевой газ</span><strong>${esc(state.config.networkFeeTon)} TON</strong></div></div></div>
      <div class="pricing-grid">${priceTierCards()}</div>
      <div class="trust-grid pricing-notes"><div class="trust-card"><h3>Зачем нужна комиссия платформы</h3><p>Комиссия нужна не только ради мотивации админов. Из неё оплачиваются технические расходы: хостинг, домен, резервный RPC, поддержка Mini App, наблюдатели за блокчейном и развитие сервиса.</p></div><div class="trust-card"><h3>Почему газ фиксирован отдельно</h3><p>Сбор ${esc(state.config.networkFeeTon)} TON — это не наша прихоть. Его требует сама сеть TON: смарт-контракт нужно развернуть, а затем выполнить подтверждение, отмену или решение спора. Неиспользованный остаток газа возвращается на служебный кошелёк платформы.</p><a class="text-link" href="${esc(state.config.gasInfoUrl || "https://docs.ton.org/contracts/techniques/gas")}" target="_blank" rel="noopener">Официальная статья TON про gas <span>→</span></a></div></div>
      <div class="panel panel-blueprint fee-explainer"><div class="fee-explainer-head"><div><h3>Быстрый калькулятор</h3><p class="page-intro">Введите сумму сделки — калькулятор сразу покажет итоговую оплату и саму переплату в обоих популярных сценариях.</p></div><div class="fee-explainer-badge">2 сценария расчёта</div></div><div class="field"><label for="quick-fee-amount">Сумма, которую готовы потратить / получить</label><input id="quick-fee-amount" inputmode="decimal" placeholder="Например, 4.5"></div><div id="quick-fee-calc" class="fee-calc" aria-live="polite"><p class="fee-empty">Введите сумму, чтобы увидеть расчёт комиссии платформы и газа.</p></div></div></section>`;
  }
    ...
    return `<section id="trust" class="trust-section">
      ...
      <div class="trust-grid">
        <div class="trust-card">
          ...
          <p>Здесь работает эскроу-логика: разворачивает контракты сделок и подтверждает выплаты. Комиссия платформы на нём не оседает — она уходит на отдельный адрес, скрытый из мини-аппа из соображений приватности, но при желании его можно запросить у поддержки.</p>
  ...
  function nextStepForDeal(deal) {
    ...
    if (deal.status === "deployed" && deal.role === "buyer") {
      const breakdown = deal.fees.feeFromSeller
        ? `Сумма сделки ${deal.amountTon} TON + сетевой сбор ${deal.fees.networkFeeTon} TON (комиссия платформы ${deal.fees.serviceFeeTon} TON удержится из выплаты продавцу).`
        : `Сумма сделки ${deal.amountTon} TON + комиссия платформы ${deal.fees.serviceFeeTon} TON + сетевой сбор ${deal.fees.networkFeeTon} TON.`;
    ...
    if (quickAmountInput) {
      const updateQuickCalc = () => {
        ...
        document.querySelectorAll(".price-card").forEach((card) => {
          card.classList.toggle("is-active", !!tier && card.dataset.tierKey === tier.key);
        });
        target.innerHTML = units && units > 0n ? calculatorVariants(units) : `<p class="fee-empty">Введите сумму — увидите обе модели оплаты комиссии и готовые итоговые суммы.</p>`;
      };
  ...
  function dealAction(action) {
    ...
    if (action === "confirm") {
      ...
      return showModal("Подтвердить получение?", `После подтверждения контракт необратимо отправит продавцу ${deal.fees.sellerPayoutTon} TON. ${feeNote}`, "Да, всё получено", () => postAction("confirm"));
    }
