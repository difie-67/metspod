(() => {
  ...
  function giftGlyph() {
    return `<svg viewBox="0 0 40 40" width="1em" height="1em" aria-hidden="true"><rect x="7" y="17" width="26" height="16" rx="1.5" fill="none" stroke="currentColor" stroke-width="2"/><path d="M5 11h30v6H5z" fill="none" stroke="currentColor" stroke-width="2" stroke-linejoin="round"/><path d="M20 11v22" stroke="currentColor" stroke-width="2"/><path d="M20 11c0-4-3-7-6.5-7S9 7 12 9c2 1.4 5 2 8 2Zm0 0c0-4 3-7 6.5-7S31 7 28 9c-2 1.4-5 2-8 2Z" fill="none" stroke="currentColor" stroke-width="2" stroke-linejoin="round"/></svg>`;
  }
  ...
  function homeView() {
    ...
    return `<section class="hero-banner">
      <div class="hero-scene" aria-hidden="true">
        <img class="hero-hand hero-hand-left" src="/miniapp/assets/hero-hand-left.png" alt="" />
        <img class="hero-hand hero-hand-right" src="/miniapp/assets/hero-hand-right.png" alt="" />
      </div>
      <div class="hero-copy">
        <span class="hero-mark-sm" aria-hidden="true"><img src="/miniapp/logo-mark.png" alt="" width="20" height="20" /></span>
        <p class="hero-eyebrow">TON · SMART CONTRACT ESCROW</p>
        <h1>Безопасные сделки в TON</h1>
        <p>Деньги хранятся в отдельном смарт-контракте сделки и выпускаются только по её правилам.</p>
        <div class="hero-actions"><button class="btn btn-primary" data-route="create">Создать сделку</button><button class="btn btn-secondary" data-route="deals">Мои сделки</button></div>
      </div>
    </section>
      ...
      ${editorial([["Отдельный контракт", "Каждая сделка получает собственный on-chain адрес."], ["Фиксированные стороны", "Адреса покупателя, продавца и комиссий записаны при создании."], ["Проверяемо", "Баланс и операции доступны в Tonviewer и Tonscan."], ["Арбитраж", "После оплаты спор решается только в пользу заданных участников."]])}`;
  }
  ...
  function createView() {
    return `<h1 class="page-title">Новая сделка</h1><p class="page-intro">Укажите условия. После создания отправьте одноразовую ссылку второй стороне.</p><form id="create-form" class="panel panel-blueprint">
      ...
      <div class="field"><label for="amount">Сумма сделки в TON (получит продавец)</label><input id="amount" name="amount" inputmode="decimal" placeholder="0,01" required></div><p class="hint">Лимит: ${esc(state.config.minDealTon)}–${esc(state.config.maxDealTon)} TON. Сверху покупатель оплачивает комиссию сервиса ${state.config.feePercent}% и сетевой сбор — он покрывает газ блокчейна.</p><div id="fee-calc" class="fee-calc" aria-live="polite"></div>
  ...
  function nextStepForDeal(deal) {
    ...
    if (deal.status === "deployed" && deal.role === "buyer") {
      return { tone: "attention", label: "ШАГ 03 · ОПЛАТА", title: `Внесите ${deal.fees.totalTon} TON в escrow`, text: `Сумма сделки ${deal.amountTon} TON + комиссия сервиса ${deal.fees.serviceFeeTon} TON + сетевой сбор ${deal.fees.networkFeeTon} TON. Используйте кнопку ниже: сумма и адрес контракта будут подставлены автоматически.`, action: `<button class="btn btn-primary btn-block" data-action="pay">Оплатить ${esc(deal.fees.totalTon)} TON</button>` };
    }
  ...
  function dealView(deal) {
    ...
    return `<button class="text-button" data-route="deals">← Все сделки</button><section class="detail-head"><span class="badge ${deal.status}">${esc(deal.statusLabel)}</span><h1>${esc(deal.amountTon)} TON</h1><p>Сделка #${deal.id} · ${deal.role === "buyer" ? "вы покупатель" : deal.role === "seller" ? "вы продавец" : "режим администратора"}</p></section>
      ...
      <div class="panel"><h3>Расчёт по сделке</h3>${feeTable({ amount: BigInt(deal.amountUnits), serviceFee: toUnits(deal.fees.serviceFeeTon), networkFee: toUnits(deal.fees.networkFeeTon), total: BigInt(deal.fees.totalUnits) }, deal.fees.feePercent)}</div>
  ...
  // Indicator positions: home/deals/transparency/profile map directly; "create"
  // has no slot of its own (it's the raised center button), so the indicator
  // just fades out and keeps whatever --tab-index it last had, ready to slide
  // in again from the correct neighbour once the user leaves "create".
  const TAB_ROUTES = ["home", "deals", null, "transparency", "profile"];
  function updateTabIndicator() {
    const indicator = document.querySelector(".tabbar-indicator");
    if (!indicator) return;
    const route = state.route === "deal" ? "deals" : state.route;
    const index = TAB_ROUTES.indexOf(route);
    if (index !== -1) indicator.style.setProperty("--tab-index", String(index));
    indicator.classList.toggle("hide", index === -1);
  }
  ...
  function bindCurrentView() {
    ...
    if (amountInput) {
      const update = () => {
        ...
        target.innerHTML = units && units > 0n ? feeTable(feeRows(units, state.config.feePercent, state.config.networkFeeTon), state.config.feePercent) : `<p class="fee-empty">Введите сумму — покажем, сколько заплатит покупатель.</p>`;
      };
  ...
  function dealAction(action) {
    ...
    if (action === "confirm") return showModal("Подтвердить получение?", `После подтверждения контракт необратимо отправит продавцу ${state.selectedDeal.amountTon} TON, а комиссия сервиса ${state.selectedDeal.fees.serviceFeeTon} TON будет удержана.`, "Да, всё получено", () => postAction("confirm"));
