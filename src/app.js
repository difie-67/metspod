(() => {
  const UI_BUILD_ID = "2026-09-25_numbered-spine";
  ...
  const editorial = (items) => `<ol class="ed-list">${items.map((item, index) => `<li class="ed-item"><div class="ed-copy"><h3>${esc(item[0])}</h3><p>${esc(item[1])}</p></div><span class="ed-no">${String(index + 1).padStart(2, "0")}</span></li>`).join("")}</ol>`;
  ...
  // Numbered "spine" lists: light the nodes up once the list is on screen.
  // Persistent class, so silent re-renders just set it straight away.
  function lightEdLists(animate) {
    const lists = view.querySelectorAll(".ed-list:not(.is-lit)");
    if (!lists.length) return;
    if (!animate || !("IntersectionObserver" in window)) { lists.forEach((list) => list.classList.add("is-lit")); return; }
