(() => {
  const UI_BUILD_ID = "2026-09-24_14-52_dealux7_adminlogs";
  ...
  const editorial = (items) => `<ol class="ed-list">${items.map((item, index) => `<li class="ed-item"><div class="ed-copy"><span class="ed-kicker">STEP</span><h3>${esc(item[0])}</h3><p>${esc(item[1])}</p></div><span class="ed-no">${String(index + 1).padStart(2, "0")}</span></li>`).join("")}</ol>`;
