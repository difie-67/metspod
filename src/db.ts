...
CREATE TABLE IF NOT EXISTS deals (
  ...
  fee_payer TEXT NOT NULL DEFAULT 'seller',
...
// Safe in-place migration for databases created before commission payer was added.
const dealColumns = db.prepare(`PRAGMA table_info(deals)`).all() as unknown as Array<{ name: string }>;
if (!dealColumns.some((column) => column.name === "fee_payer")) {
  db.exec(`ALTER TABLE deals ADD COLUMN fee_payer TEXT NOT NULL DEFAULT 'seller'`);
}
...
export function createDeal(params: {
  ...
  const info = db.prepare(
    `INSERT INTO deals (
      status, creator_tg_id, buyer_tg_id, seller_tg_id, amount_units, fee_payer,
