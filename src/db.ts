...
import { config, feeThresholdUnits, networkFeeUnits } from "./config";
import { feeBpsForAmount } from "./fees";
...
// Fee terms are frozen per deal because they are part of the contract init data
// (and therefore of the contract address). Nullable for pre-existing rows.
// buyer_consent/seller_consent are tri-state: NULL = not asked yet, 1 = agreed
// to show their username in the public deal history, 0 = declined.
// fee_from_seller: 0 (default/legacy) = buyer pays the fee on top, 1 = the
// fee is deducted from the seller's payout instead. Frozen per deal like the
// other fee terms, since it's baked into the contract's init data.
for (const column of [
  "fee_bps INTEGER",
  "network_fee_units TEXT",
  "fee_from_seller INTEGER",
  "buyer_consent INTEGER",
  "seller_consent INTEGER",
  "consent_asked_at INTEGER",
]) {
  try { db.exec(`ALTER TABLE deals ADD COLUMN ${column}`); } catch { /* already exists */ }
}
...
export interface DealRow {
  ...
  buyer_consent: number | null;
  seller_consent: number | null;
  consent_asked_at: number | null;
  created_at: number;
  updated_at: number;
}
...
export function createDeal(params: {
  ...
  amountUnits: bigint;
  description: string;
  /** For fee-bearing deals: true = deduct service fee from seller, false = buyer pays it on top. */
  ...
  // Freeze the fee rule into the deal. 7 TON exactly is fee-free; only amounts
  // strictly above the configured threshold receive the percentage fee.
  const frozenFeeBps = feeBpsForAmount(params.amountUnits, config.feeBps, feeThresholdUnits);
  const frozenFeeFromSeller = frozenFeeBps > 0 && params.feeFromSeller ? 1 : 0;

  const info = db.prepare(
    `INSERT INTO deals (
      status, creator_tg_id, buyer_tg_id, seller_tg_id, amount_units,
      description, expected_counterparty_username, invite_token, fee_bps, network_fee_units, fee_from_seller, created_at, updated_at
    ) VALUES ('draft', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
    params.creatorTgId,
    buyerTgId,
...
export function setDealConsent(dealId: number, tgId: number, consent: boolean): boolean {
  const deal = getDeal(dealId);
  if (!deal) return false;
  const value = consent ? 1 : 0;
  if (deal.buyer_tg_id === tgId) {
    db.prepare(`UPDATE deals SET buyer_consent = ? WHERE deal_id = ?`).run(value, dealId);
    return true;
  }
  if (deal.seller_tg_id === tgId) {
    db.prepare(`UPDATE deals SET seller_consent = ? WHERE deal_id = ?`).run(value, dealId);
    return true;
  }
  return false;
}

export function markConsentAsked(dealId: number) {
  db.prepare(`UPDATE deals SET consent_asked_at = ? WHERE deal_id = ?`)
    .run(Math.floor(Date.now() / 1000), dealId);
}

// Deals visible in the public, no-login history: only ones that actually
// reached the chain (have a contract address) and reached a final state, so
// the list only ever shows real, verifiable on-chain outcomes.
export function getPublicDeals(limit = 200): DealRow[] {
  const safeLimit = Math.max(1, Math.min(500, Math.trunc(limit)));
  return db.prepare(
    `SELECT * FROM deals
     WHERE contract_address IS NOT NULL AND status IN ('completed', 'cancelled', 'resolved')
     ORDER BY deal_id DESC LIMIT ${safeLimit}`
  ).all() as unknown as DealRow[];
}

export function countPublicDeals(): number {
  const row = db.prepare(
    `SELECT COUNT(*) AS count FROM deals WHERE contract_address IS NOT NULL AND status IN ('completed', 'cancelled', 'resolved')`
  ).get() as { count: number };
  return Number(row.count);
}
