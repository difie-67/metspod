...
import { getRecentLogs, getServerStartedAt } from "./logger";
...
function sha256File(relativePath: string): string | null {
  const found = resolveProjectFile(relativePath);
  if (!found) return null;
  return crypto.createHash("sha256").update(fs.readFileSync(found)).digest("hex");
}

async function adminDiagnostics() {
  const startedAt = getServerStartedAt();
  const serviceInfo = await getServiceWalletInfo().catch(() => null);
  const fallbackServiceAddress = config.expectedServiceWalletAddress || "";
  const serviceAddress = serviceInfo ? formatAddress(serviceInfo.address) : fallbackServiceAddress;
  const codeHash = await getEscrowCodeHash().catch(() => "unavailable");
  return {
    ...
    adminIds: config.arbiterTgIds.join(", "),
    platformAddress: config.platformAddress,
    contract: {
      codeHash,
      verifierUrl: config.verifierUrl,
      sourceSha256: sha256File("contracts/escrow.tact"),
      tonModuleSha256: sha256File("src/ton.ts"),
      lockfileSha256: sha256File("package-lock.json"),
      buildManifestSha256: sha256File("BUILD_VERSION.txt"),
    },
    serviceWallet: {
      address: serviceAddress,
      balanceTon: serviceInfo ? formatTonAmount(serviceInfo.balance) : null,
      state: serviceInfo?.state || "unavailable",
      explorers: serviceAddress ? explorerUrls(serviceAddress) : { tonviewer: "", tonscan: "" },
    },
  ...
  if (method === "GET" && url.pathname === "/api/bootstrap") {
    ...
    return json(res, 200, {
      ...
      admin: isAdmin && stats ? {
        ...
        diagnostics: await adminDiagnostics(),
      } : null,
    });
  }
