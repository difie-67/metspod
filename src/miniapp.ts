...
function serializeDeal(deal: DealRow, viewerId: number, chainState: number | null = null) {
  ...
  const amountUnits = BigInt(deal.amount_units);
  const feeUnits = (amountUnits * BigInt(config.feeBps)) / 10000n;
  const paymentUnits = amountUnits + (deal.fee_payer === "buyer" ? feeUnits : 0n);
  const sellerReceivesUnits = amountUnits - (deal.fee_payer === "seller" ? feeUnits : 0n);
  return {
    id: deal.deal_id,
    status: deal.status,
    ...
    feeTon: formatTonAmount(feeUnits),
    paymentUnits: paymentUnits.toString(),
    paymentTon: formatTonAmount(paymentUnits),
    sellerReceivesTon: formatTonAmount(sellerReceivesUnits),
...
async function handleAction(deal: DealRow, user: TelegramUser, action: string, body: Record<string, unknown>) {
  ...
  if (action === "confirm") {
    if (deal.buyer_tg_id !== user.id) throw new Error("Подтвердить получение может только покупатель");
    if (deal.status !== "funded" || !deal.contract_address) throw new Error("Сделка не готова к подтверждению");
    await sendConfirm(parseAddress(deal.contract_address)!, deal.deal_id);
    setDealStatus(deal.deal_id, "confirm_pending");
    await notify([deal.buyer_tg_id, deal.seller_tg_id], `Получение по сделке #${deal.deal_id} подтверждено. Выплата отправлена в блокчейн.`);
...
async function handleApi(req: AuthenticatedRequest, res: ServerResponse, url: URL) {
  ...
  try {
    ...
    if (method === "POST" && url.pathname === "/api/deals") {
      ...
      const feePayer = body.feePayer === "buyer" || body.feePayer === "seller" ? body.feePayer : null;
      if (!feePayer) throw new Error("Выберите, кто оплачивает комиссию");
      ...
      const deal = createDeal({ creatorTgId: user.id, creatorRole: role, counterpartyUsername: rawUsername || null, amountUnits: amount, feePayer, description });
      const username = await botUsername();
      return json(res, 201, { ok: true, deal: serializeDeal(deal, user.id), inviteUrl: `https://t.me/${username}?startapp=join_${deal.invite_token}` });
    }
...
function staticRoot(): string {
  const candidates = [path.resolve(__dirname, "..", "..", "miniapp"), path.resolve(__dirname, "..", "miniapp")];
...
function serveStatic(res: ServerResponse, pathname: string) {
  ...
  const relative = pathname === "/" ? "index.html" : pathname.replace(/^\/miniapp\//, "").replace(/^\//, "");
  ...
  res.writeHead(200, {
    ...
    "content-security-policy": "default-src 'self'; script-src 'self' https://telegram.org; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; connect-src 'self' https: wss:; frame-src https:; base-uri 'none'; form-action 'self'",
  });
...
export function startMiniAppServer() {
  const server = http.createServer(async (req: AuthenticatedRequest, res) => {
    try {
      ...
      if (url.pathname === "/health") return json(res, 200, { ok: true, service: "obra-guarant", network: config.tonNetwork });
