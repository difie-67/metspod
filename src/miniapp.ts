...
import { getRuntimeLogPath, getRuntimeLogTail, getStartupAt, readBuildVersionNote } from "./logger";
...
if (method === "GET" && url.pathname === "/api/admin/diagnostics") {
  if (!isAdmin) return fail(res, 403, "Доступно только администратору");
  const proto = String(req.headers["x-forwarded-proto"] || "https");
  const host = String(req.headers.host || "");
  const baseUrl = host ? `${proto}://${host}` : (config.miniAppUrl || "");
  const serviceInfo = await getServiceWalletInfo().catch((error) => ({
    error: error instanceof Error ? error.message : String(error),
  }));
  const serviceAddress = "address" in serviceInfo ? formatAddress(serviceInfo.address) : (config.expectedServiceWalletAddress || null);
  return json(res, 200, {
    ok: true,
    build: APP_BUILD_ID,
    startupAt: getStartupAt(),
    generatedAt: new Date().toISOString(),
    note: "Внутри Mini App доступны текущая версия сборки, runtime-логи приложения и параметры контракта. Полные исторические build/deploy-логи Railway остаются в Railway UI/API.",
    links: {
      version: baseUrl ? `${baseUrl}/version` : "/version",
      health: baseUrl ? `${baseUrl}/health` : "/health",
    },
    buildNote: readBuildVersionNote(),
    runtime: {
      logPath: getRuntimeLogPath(),
      logs: getRuntimeLogTail(160),
    },
    environment: {
      network: config.tonNetwork,
      ...
      adminIds: config.arbiterTgIds,
    },
    serviceWallet: {
      address: serviceAddress,
      balanceTon: "balance" in serviceInfo ? formatTonAmount(serviceInfo.balance) : null,
      state: "state" in serviceInfo ? serviceInfo.state : null,
      error: "error" in serviceInfo ? serviceInfo.error : null,
      explorers: serviceAddress ? explorerUrls(serviceAddress) : null,
    },
    contract: {
      escrowCodeHash: await getEscrowCodeHash(),
      verifierUrl: config.verifierUrl || null,
      gasInfoUrl: config.gasInfoUrl || null,
    },
  });
}
